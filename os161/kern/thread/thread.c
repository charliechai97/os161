...
struct threadnode processList[200];

int exitCodesArray[200];
...
void setExit(int exitCode, pid_t pid){
	
	exitCodesArray[pid] = exitCode;
}
int getExit(pid_t pid){
	return exitCodesArray[pid];
}
...
thread_create(const char *name) {
	...
	thread->waitOnMe =sem_create("sem",0);
	//thread->need2Check =sem_create("sem2",0);
	//thread->imParent =sem_create("sem3",0);
	//P(thread->waitOnMe);
	thread->exited = 0;
	thread->beenWaitedOn =0;
...
struct thread* findHead (struct thread* thread){
	if(thread->parent == NULL)
		return thread;
	else
		return findHead(thread->parent);
}
struct thread* findThread (pid_t pid, struct thread* thread){
	...
	if(thread->pid == pid){
		...
		for(i = 0; i < 100; i++){
			if(thread->children[i] != NULL){
				kprintf("before: %d\n",i);
				return findThread(pid,thread->children[i]);
				kprintf("after: %d\n",i);
			}
		}
		return NULL;
...
struct thread* doesThreadExist (pid_t pid){
	//struct thread* head = findHead(thread);
	//struct thread* found = findThread(pid,head);
	//return found;
	int spl = splhigh();
	int i;
	for(i = 0; i < 200; i++){
		if (processList[i].my_thread != NULL){
			//kprintf("before: %d %d\n",processList[i]->pid,pid);
		
			if(processList[i].pid == pid){
				splx(spl);
				//kprintf("befodfsdre: %d\n",i);
				return processList[i].my_thread;
			}
			//kprintf("after: %d %d\n",processList[i]->pid,pid);
		}
	}
...
struct thread* getThread(pid_t pid){
	//kprintf("\ngetthread: %d %d\n", processList[pid].pid, pid);
	return processList[pid].my_thread;
}
...
thread_fork(const char *name,
    ...
    //for(i = 0; i < 100; i++){
	newguy->pid = totalPid;
	processList[totalPid].pid=totalPid;
	//}

	//processList[totalPid] = newguy;
	processList[totalPid].my_thread = newguy;
	//kprintf("\nmy pid is: %d %d\n", processList[totalPid].pid, totalPid);

	//for(i = 0; i < 200; i++){
	//	if(processList[i] == NULL){
	//		processList[i] = newguy;
	//		break;
	//	}
	//}

	/*for(i = 1; i <= totalPid; i++){
		if(processList[i].my_thread != NULL)
			kprintf("\my new pid is: %d %d\n", processList[i].pid, i);
	}*/

//
	processList[totalPid].waitOnMe =sem_create("sem",0);
	//thread->need2Check =sem_create("sem2",0);
	//thread->imParent =sem_create("sem3",0);
	//P(thread->waitOnMe);
	processList[totalPid].exited = 0;
	processList[totalPid].beenWaitedOn =0;

//
	processList[totalPid].parent = curthread;
	totalPid++;
	...
	for(i = 0; i < 100; i++){
		if(curthread->children[i] == NULL){
			curthread->children[i] = newguy;
			break;
		}
	}
...
mi_switch(threadstate_t nextstate) {
    ...
    if (curthread != NULL && curthread->t_stack != NULL) {
         ...
         */
	//kprintf("curthread stack 0: %x\n",curthread->t_stack[0]);
	//kprintf("curthread stack 1: %x\n",curthread->t_stack[1]);
	//kprintf("curthread stack 2: %x\n",curthread->t_stack[2]);
	//kprintf("curthread stack 3: %x\n",curthread->t_stack[3]);
	//kprintf("curthread: %x\n",curthread);
...
thread_exit(void) {
	...
	//clear from process table
	//int i;
	//for(i = 0; i < 200; i++){
	//	if(processList[i] != NULL)
	//		if(processList[i]->pid == curthread->pid){
	//			processList[i] = NULL;
	//			break;
	//		}
//	}
