...
cmd_progthread(void *ptr, unsigned long nargs)
{
	...
	result = runprogram(progname,ptr,nargs);
	if (result) {
...
common_prog(int nargs, char **args)
{
	...
	result = thread_fork(args[0] /* thread name */,
			...
			cmd_progthread, &theThread);
	if (result) {
		kprintf("thread_fork failed: %s\n", strerror(result));
		return result;
	}
	result = sys_waitpid(theThread->pid,&status,0,&error);
	if (error)
		return error;
...
{
	int flagger = atoi(args[1]);
	if(flagger > 12 || flagger < 1)
	{
